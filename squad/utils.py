...
#defined by Snigdha to get all spans and their scores (no Dynamic Programming)
def get_all_spans_Snigdha(ypi, yp2i, context, wordss, max_span_len):
    d = dict()
    for f, (ypif, yp2if) in enumerate(zip(ypi, yp2i)):
        argmax_j1 = 0
        for s in range(len(ypif)):
            val1 = ypif[s]
            for e in range(s,len(yp2if)):
                if((e-s)<max_span_len):
                    val2 = yp2if[e]
                    score = val1 * val2
                    sent_idx = f
                    span = ((sent_idx, s), (sent_idx, e + 1))
                    d[span] = float(score)
    return d

#defined by Snigdha
def get_best_span_Snigdha(ypi, yp2i, seen):
    max_val = 0
    best_word_span = (0, 1)
    best_sent_idx = 0
    for f, (ypif, yp2if) in enumerate(zip(ypi, yp2i)):
        ...
        for j in range(len(ypif)):
            ...
            val2 = yp2if[j]
            if val1 * val2 > max_val:
                word_span = (argmax_j1, j)
                if(not inSeen(word_span, f, seen)): #Snigdha
                    best_word_span = word_span
                    best_sent_idx = f
                    max_val = val1 * val2
    return ((best_sent_idx, best_word_span[0]), (best_sent_idx, best_word_span[1] + 1)), float(max_val)
	
# defined by Snigdha
def inSeen(best_word_span, best_sent_idx, seen):
    #Seen is a list of pairs
    p = ((best_sent_idx, best_word_span[0]), (best_sent_idx, best_word_span[1] + 1))
    for q in seen:
        if (p[0][0]==q[0][0]) and (p[0][1]==q[0][1]) and (p[1][0]==q[1][0]) and (p[1][1]==q[1][1]):
            return True
    return False
